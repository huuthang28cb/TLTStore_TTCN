...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!user) {
        return next(new ErrorHandler('Không tìm thấy Email người dùng', 404));
    }
    ...
    const message = `Bạn muốn thay đổi mật khẩu:\n\n${resetUrl}\n\nNếu bạn không yêu cầu email này, thì hãy bỏ qua nó.`

    try {

        await sendEmail({
            email: user.email,
            subject: 'Shop TLT cập nhật mật khẩu',
            message
        })

        res.status(200).json({
            success: true,
            message: `Email gửi tới: ${user.email}`
        })
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!user) {
        return next(new ErrorHandler('Mã xác nhận đặt lại mật khẩu không hợp lệ hoặc đã hết hạn', 400))
    }
